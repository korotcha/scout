  ...
  return <TooltipProvider delayDuration={350} skipDelayDuration={500}><div className={`query-table-shell ${compact ? "query-table-compact" : "query-table-screener"} ${renderScore ? "query-table-scored" : ""} ${selection ? "query-table-selectable" : ""}`}>
    <Table>
      ...
      <TableHeader><TableRow>
        ...
        {renderScore && <TableHead scope="col" className="query-column-score" aria-sort={scoreSort === "desc" ? "descending" : scoreSort === "asc" ? "ascending" : "none"}><div className="query-header-stack"><div className="query-header-title"><HeaderHelp label="Общий балл">Оценка по фактам разбора: спрос, продажи, конкуренты и риски. Звёздочка — анализ не завершён. 0–39 — красный, 40–69 — жёлтый, от 70 — зелёный. Стоп-факторы проверяются отдельно; высокий балл не разрешает закупку.</HeaderHelp><button type="button" className="query-sort-button size-6 shrink-0" aria-label="Сортировать по общему баллу" data-active={Boolean(scoreSort)} onClick={onScoreSort}>{scoreSort === "asc" ? <ArrowUp className="size-3.5" /> : <ArrowDown className="size-3.5" />}</button></div><div className="query-header-filter-slot">{scoreFilter && <div className="query-column-filter">{scoreFilter}</div>}</div></div></TableHead>}
        <TableHead scope="col" className="query-name-head query-column-name"><div className="query-header-stack"><div className="query-header-title"><HeaderHelp label="Запрос">Фраза покупателя из исходного свода за {current}.{onOpen ? " Нажмите на запрос, чтобы открыть отдельный разбор этого запроса. Внутри доступен весь пул предмета." : " Здесь доступен загруженный пул предмета, включая запросы, не прошедшие фильтр скринера."} Прочерк вместо показателя не означает отсутствие спроса.</HeaderHelp></div><div className="query-header-filter-slot" /></div></TableHead>
        {!compact && <TableHead scope="col" className="query-column-subject"><div className="query-header-stack"><div className="query-header-title"><HeaderHelp label="Предмет WB">Приоритетный предмет, к которому Wildbox отнёс запрос в выгрузке за {current}. Классификация может быть неточной: принадлежность проверяем при разборе. Прочерк — предмет не указан.</HeaderHelp></div><div className="query-header-filter-slot" /></div></TableHead>}
        {numericColumns.map(({ key, label }) => <TableHead scope="col" className={`query-column-${key} text-right`} key={key} aria-sort={sort?.key === key ? sort.direction === "desc" ? "descending" : "ascending" : "none"}>
          <div className="query-header-stack"><div className="query-header-title query-header-sortable flex items-center justify-end gap-1"><HeaderHelp label={label}>{hints[key]}</HeaderHelp>{onSort && <button type="button" data-active={sort?.key === key} className="query-sort-button grid size-6 shrink-0 place-items-center rounded hover:bg-slate-200/60 focus-visible:outline-2" onClick={() => onSort(key)} aria-label={`Сортировать: ${label}, ${sort?.key === key && sort.direction === "desc" ? "по возрастанию" : "по убыванию"}`}>{sort?.key === key && sort.direction === "asc" ? <ArrowUp className="size-3.5" /> : <ArrowDown className="size-3.5" />}</button>}</div><div className="query-header-filter-slot">{renderFilter && <div className="query-column-filter">{renderFilter(key)}</div>}</div></div>
        </TableHead>)}
